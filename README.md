...
just `meteor add ziarno:refresh-sound` and you're good to go!

This package is intended to bring your focus back
to the page when meteor hot reload refreshes it
...
FAQ
---
Q: Can I play the sound at some other time than startup?
(ex. in some view, after rendering or subscription ready)
- A: Yes, set `RefreshSound.playAtStartUp = false` before `Meteor.startup`,
then use `RefreshSound.play()` or `RefreshSound.playOnce()` wherever you want

Q: Can I change the sound?
- A: Yes, just do `RefreshSound.sound = new Audio('data:audio/wav;base64,//your-sound')`.
If you have an mp3 sound - use an online mp3 to base64 converter,
then paste the base64 string into the `Audio` object